...
public sealed class PasswordResetTokenService(IConfiguration config)
{
    ...
    {
        var raw = config["PasswordReset:SigningKey"] ?? config["TokenKey"]
            ?? throw new InvalidOperationException("TokenKey or PasswordReset:SigningKey must be set.");
